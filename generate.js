...
export default async function handler(req, res) {
  ...
  if (!product) return res.status(400).json({ error: "Опишите продукт (поле product)" });
...
Сгенерируй РОВНО 2 рекламных креатива под разные маркетинговые углы.
Верни ТОЛЬКО валидный JSON-массив, без markdown и пояснений. Формат элемента:
{
 ...
 "bg_prompt": "детальный промпт ФОНА на английском для AI-генератора: атмосферная сцена/объект по центру, стиль современной рекламы, мягкое освещение, нужное настроение. ВАЖНО: на фоне НЕ должно быть никакого текста, букв, цифр, логотипов и кнопок (текст наложим отдельно). Оставь верх, низ и правый край относительно спокойными/тёмными для читаемости наложенного текста.",
