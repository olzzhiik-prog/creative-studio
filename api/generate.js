...
// Экспертная роль для генерации продающих УТП
const MARKETER = `Ты — главный маркетинговый стратег с мышлением уровня Филипа Котлера: позиционирование, STP, ценностное предложение, jobs-to-be-done, отстройка от конкурентов. При этом ты в совершенстве знаешь рынок Казахстана — поведение местных покупателей, доминирование Instagram, TikTok и Kaspi, чувствительность к цене, решающую роль доверия и социального доказательства, двуязычие (русский и казахский).

Ты делаешь ПРОДАЮЩИЕ УТП, а не общие фразы. Правила:
- Бей в конкретную боль или желание клиента, а не в характеристики компании.
- Конкретика и цифры вместо воды. СТРОГО ЗАПРЕЩЕНЫ пустые штампы: «качество и сервис», «индивидуальный подход», «низкие цены», «быстро и качественно», «команда профессионалов», «широкий ассортимент».
- Чёткая выгода или результат для клиента + чем вы лучше конкурентов.
- Правдоподобность: обещаешь результат — добавь срок, механизм, гарантию или доказательство.
- Живой разговорный язык под соцсети Казахстана, без канцелярита.`;
...
Черновик УТП от клиента: "${usp}"
${product ? `Продукт/услуга: "${product}"` : ""}
${audience ? `Аудитория: "${audience}"` : ""}

Сделай его ПРОДАЮЩИМ, сохранив исходный смысл и суть оффера: добавь конкретику или цифры, чёткую выгоду для клиента, снятие риска или доказательство, отстройку от конкурентов.
Дай 3 усиленных РАЗНЫХ варианта под разные рычаги. Каждый — одна ёмкая фраза до 12 слов.
...
Сформулируй 3 РАЗНЫХ сильных УТП под разные маркетинговые рычаги (например: гарантия или снятие риска, конкретный результат за срок, узкая специализация, социальное доказательство, выгодная механика — рассрочка/Kaspi). Каждое — одна ёмкая фраза до 12 слов.
Верни ТОЛЬКО валидный JSON-массив из 3 строк, без markdown и пояснений.`;
      const arr = parseJson(await geminiText(key, p));
      return res.status(200).json({ options: Array.isArray(arr) ? arr.slice(0, 3) : [] });
    }
    ...
    const copyPrompt = `Ты — элитный перформанс-маркетолог и арт-директор рекламы для соцсетей с мышлением уровня Котлера, отлично знающий рынок Казахстана (Instagram, TikTok, Kaspi, чувствительность к цене, доверие и социальное доказательство). Заголовки — продающие и конкретные, БЕЗ пустых штампов («качество и сервис», «индивидуальный подход», «быстро и качественно», «низкие цены»).
